...
for i, (name, f_ad, g_ad, f_fd, g_fd) in enumerate(models):
    ...
    ax_f.set_title(name, fontsize=7.5, fontweight="bold", x=0.4)
...
axes[0, 0].set_ylabel("Objective $f(\\theta)$", fontsize=7.5, labelpad=2)
axes[1, 0].set_ylabel("Gradient norm $\\|\\nabla f\\|$", fontsize=7.5, y=0.35, labelpad=2)

fig.tight_layout(h_pad=0.3, w_pad=0.4, rect=[0, 0, 1, 0.97])

# x-axis labels back on axes
axes[1, 0].set_xlabel("Iteration", fontsize=7.5)
axes[1, 1].set_xlabel("Iteration", fontsize=7.5)
...
leg = axes[0, 0].legend(handles=handles, loc="upper right", fontsize=6.5,
