...
2. **Figure 6** (`fig7_resource_efficiency/`) -- observed AD-vs-FD speedup
...
- Uses `fig4_wallclock/{model}/run.py` as the underlying benchmark script
  (each inner script `cd`s into the corresponding fig2 directory)
- Requires `minimum_resources_comparison.csv` from `fig7_resource_efficiency/`
