...
def create_model():
    ...
    np.random.seed(4)
    if args.theta_initial is not None:
        theta_initial = np.array(args.theta_initial)
    else:
...
if __name__ == "__main__":
    print_msg("--- wa2: Coregional (3-variate) Spatio-Temporal Model (distributed, 4 nodes) ---")
    ...
    if args.benchmark_mode:
        ...
        run_benchmark(dalia_fd, dalia_jax, args, "wa2")
        exit(0)
    ...
    if not args.skip_jax:
        ...
        write_optimization_result(
            args.results_csv, "wa2", "jax_autodiff", results_jax,
    ...
    if not args.skip_fd:
        ...
        write_optimization_result(
            args.results_csv, "wa2", "finite_diff", results_fd,
