...
DATA_DIR = os.path.join(ARTIFACT_DIR, "data", "scaling_wa2_spatial")

NV = 3
NS = int(os.environ.get("NS", 1119))
NT = 48
NB = 3
DIM_THETA = 15
...
def create_model():
    """Create the trivariate coregional model for WA2 spatial scaling."""
    theta_ref_file = (
...
if __name__ == "__main__":
    ...
    print_msg("--- fig5: WA2 Spatial Scaling ---")
    print_msg(f"--- WA2: Trivariate Coregional (ns={NS}, nt={NT}, N={N_latent:,}) ---\n")

    model, theta_ref = create_model()
    print_msg(model)

    if args.benchmark_mode:
        model_label = f"WA2-ns{NS}"
        fd_results = None
        jax_results = None
